...
int calcMatCovar(const double* const* matXs, double** matCovar, int dim, int num)
{
	int d = 0;
	int d1 = 0;
	int d2 = 0;
	...
	for (n = 0; n < num; ++n)
	{
		for (d = 0; d < dim; ++d)
		{
			matDif[n][d] = matXs[n][d] - matXEven[d][0];
		}
	}
...
int main()
{
	...
	if (calcMahaDistance(matXs[0], matXs[1], matCovarInv, dim, &dist) < 0)
		return -1;
